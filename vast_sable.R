...
library(here)           #don't think I need to port into HB
library(tidyverse)
library(sf)
library(scales)
library(renv)           #don't think I need to port into HB
library(lubridate)
library(janitor)
library(VAST)
...
# species <- 
run_version <- "sable_only" # 'sable_only' 'sable-squid'
this_year <- lubridate::year(lubridate::today())

speciesName <- paste0("Sablefish_",this_year,"_", run_version)
...
# #start thorson change
# base::load(here::here("all_years_sable.RData"))
# workDir <- here::here("VAST_results", speciesName)
# 
# user_region$Lon = ifelse( user_region$Lon>0, user_region$Lon-180, user_region$Lon+180)
# settings_quick$max_cells = 2000
# end thorson change
...
# options(max.print = .Machine$integer.max)
# 
# colnames(full_fit$extrapolation_list$a_el) = c("Total", "EBS", "NBS")
# 
# # Naming
...
# dim(Cov_Index)
# 
# # v_cov <- full_fit$parameter_estimates$SD$cov %>% as.data.frame()
